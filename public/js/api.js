...
const api = {
  ...
  // Campaña — presupuesto, gastos, caja, balance
  camp: {
    balance:            ()       => apiFetch('/logistica/balance'),
    presupuestos:       ()       => apiFetch('/logistica/presupuestos'),
    crearPresupuesto:   (body)   => apiFetch('/logistica/presupuestos',     { method: 'POST',   body }),
    editarPresupuesto:  (id, b)  => apiFetch('/logistica/presupuestos/'+id, { method: 'PUT',    body: b }),
    borrarPresupuesto:  (id)     => apiFetch('/logistica/presupuestos/'+id, { method: 'DELETE' }),
    gastos:             (p = {}) => apiFetch('/logistica/gastos?' + new URLSearchParams(p)),
    crearGastoCamp:     (body)   => apiFetch('/logistica/gastos',           { method: 'POST',   body }),
    borrarGasto:        (id)     => apiFetch('/logistica/gastos/'+id,       { method: 'DELETE' }),
